...
A total of 10 major bugs were identified through exploratory testing. However, it is difficult to have repeatability with exploratory testing, since there is no predefined script to follow. Due to the lack of repeatability, exploratory testing is not suited for regression testing. 

Manual functional testing involves the tester following a set of scripted tests. These tests can cover most functionalities and can be repeated after any changes to the software are made. Since they are scripted, they can generally be completed within a known time limit and by testers with less experience. Over multiple test cycles, manual functional testing becomes more efficient. The repeatable nature of scripted tests makes them well-suited for regression testing new software releases.
